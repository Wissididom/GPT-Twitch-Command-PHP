...
if (isset($_GET['prompt'])) {
	...
	curl_setopt($ch, CURLOPT_POSTFIELDS, json_encode([
		'message' => '🤖' . substr($response['choices'][0]['message']['content'], 0, 440) // StreamElements seems to prevent messages of more than 500 chars. It seems like the limit is around 450
	]));
	curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
	$response = json_decode(curl_exec($ch), true);
	curl_close($ch);
